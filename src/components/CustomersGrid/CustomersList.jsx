...
const CustomersList = () => {
  ...
  useEffect(() => {
    const get_data = async () => {
      await axios.get("https://kreifeur-goujil.onrender.com/customers").then((res) => setData(res.data));
    };
    get_data();
  }, [data]);
  ...
  const deletecustomer = (id) => {
    axios
      .delete(`https://kreifeur-goujil.onrender.com/customer/${id}`)
      .then((res) => console.log(res));
  };

  const sendupdatedata = () => {
    axios
      .put(`https://kreifeur-goujil.onrender.com/customer/${updata.id}`, updata)
      .then((res) => console.log(res));
    setVis(false);
  };
  ...
  return (
    <div className="bg-white rounded col-span-4 row-span-4 py-2 overflow-x-auto">
      <div className="px-2">
        ...
        <div className="flex gap-3 bg-gray-100 items-center rounded-xl px-1">
        ...
        {vis ? (
          <form onSubmit={sendupdatedata} className="border p-1 justify-between flex gap-2">
            ...
            <input
              value={updata.taamol}
              onChange={(e) => {
                setUpdata({ ...updata, taamol: e.target.value });
              }}
              className="border px-1 outline-none"
              type="date"
            />
            ...
            {filteredData.map((e) => {
              return (
                <tr key={e.id} className="bg-white border-b ">
                  ...
                  <td className="px-6 py-4">{e.adress}</td>
                  <td className="px-6 py-4">{e.productslist}</td>
                  <td className="px-6 py-4">{e.taamol}</td>
